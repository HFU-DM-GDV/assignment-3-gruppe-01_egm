...
def __click_src(event, x, y, flags, original):
    ...
    # If the left mouse button was clicked, add the point to the original image array
...
def __click_dst(event, x, y, flags, transformee):
    ...
    # If the left mouse button was clicked, add the point to the transformee image array
    if event == cv2.EVENT_LBUTTONDOWN:
...
def guided_image_transform(master_img, transformee_img) -> np.ndarray:
    ...
    choice = input(
        "Would you like to go through a guided transformation to achieve this? (y/n)\n")
    if choice == 'n':
    ...
    cv2.setWindowProperty("Master image", cv2.WND_PROP_TOPMOST, 1)
    cv2.setWindowProperty("Transformee image", cv2.WND_PROP_TOPMOST, 1)
    cv2.setWindowProperty("Transformed image", cv2.WND_PROP_TOPMOST, 1)
    ...
    while True:
        ...
        if not (computationDone) and (len(__ref_pt_original) == 3 and len(__ref_pt_transformed) == 3):
            T_affine = cv2.getAffineTransform(np.float32(
                __ref_pt_transformed), np.float32(__ref_pt_original))

            print("\nAffine transformation:\n",
                  "\n".join(["\t".join(["%03.3f" % cell for cell in row]) for row in T_affine]))

            dst_transform = cv2.warpAffine(
                transformee_img_clean, T_affine, (cols, rows))
        ...
        if key == ord("r"):
            ...
            computationDone = False
            cv2.setMouseCallback("Master image", __click_src, master_img)
            cv2.setMouseCallback("Transformee image",
                                 __click_dst, transformee_img)
